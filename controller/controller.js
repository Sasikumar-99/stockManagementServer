...
const  { LoginSchema,ProductsSchema,ProductItemSchema,chatsArchive,chatItemHistory } = require("../model/model");
...
const createUserCollection = async(req,res,next)=>{
    try{
        ...
        if(!existingUser && user.IsRegister){
          ...
          if(createProducts && AssignChat){
            ...
            res.status(500).json({
              error : true,
              message : "something went wrong while creating products or creating chat",
            })
          }
  ...
  const chatEmitted = async (data)=>{
    if(this.inChatUsers){
      ...
      const chattingUserChatData = await chatsArchive.findOne({_id:data.to.chatId})
      if(loggedUserChatData && chattingUserChatData){
        ...
        const updatedLoggedUserChatData = await chatsArchive.findOneAndUpdate({_id:chatId},{
          chatsArchive: loggedUserChatData.chatsArchive
        },{new:true})
        const updatedChattingUserData = await chatsArchive.findByIdAndUpdate({_id:data.to.chatId},{
        ...
        if(updatedLoggedUserChatData && updatedChattingUserData){
          const response = {
            ...
            body : {updatedLoggedUserChatData:updatedLoggedUserChatData,updatedChattingUserData:updatedChattingUserData}
          }
          return response
        }else{
          const response = {
            error : true,
            statusCode : 500,
            message : 'could not update updatedLoggedUserChatData OR updatedChattingUserData'
          }
          return response
        } 
      }else{
  ...
  module.exports = {createUserCollection,postProduct,getAllProducts,deleteProducts,editProducts,updateLogin,updateProductsId,
    chattingUsers,chatEmitted,getAllUsers}